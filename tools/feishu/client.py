...
def send_card_message(
    *,
    ...
    """发送飞书交互式卡片消息（msg_type: interactive）。"""
    if not tenant_access_token:
        raise FeishuError("缺少 tenant_access_token。")
    if not receive_id:
        raise FeishuError("缺少 receive_id。")
    ...
    try:
        ...
        raise FeishuError(f"发送飞书卡片消息失败: {exc}") from exc
    try:
        obj = resp.json()
    except Exception as exc:
        raise FeishuError(f"飞书卡片消息响应非 JSON: {resp.text[:240]!r}") from exc
    if int(obj.get("code", 0)) != 0:
        raise FeishuError(f"发送飞书卡片消息失败: {obj}")
    return obj
