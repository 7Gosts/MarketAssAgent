...
TASK_PROMPTS = {
    ...
    "market_view": "用户想看行情，请客观分析当前结构、关键位和可能走势；若方向与关键位共振、机会较明确，补充条件化操作建议（入场触发、止损、失效条件），否则说明观察条件。",
    "comparison": "用户希望对比多个标的，请给出差异、优劣和风险点；对比后若相对强弱或某一标的的单边机会较明确，给出优先标的及条件化操作建议，否则说明双方均宜观望及各自触发条件。",
