...
- 风险提示放在最后，并使用 `>` 引用格式。
- 不要输出 JSON，不要输出模板占位符。
- 工具 structure_signals 是结构事实，不是概率；不要向用户说「置信度 XX%」。
- 分析/对比完成后：机会较明确时补充条件化【操作建议】（入场触发、止损、失效）；方向不明则写【观察条件】，不要硬给开仓计划。
- 默认周期：加密货币 4h，其他标的 1d；用户未要求多周期时，单标的只分析一次，不要自行做「日线+4H」。"""
...
TASK_PROMPTS = {
    ...
    "market_view": "用户想看行情，请客观分析当前结构、关键位和可能走势；默认周期：crypto 4h、其他 1d，未要求多周期时只调一次 analyze_market；若方向与关键位共振、机会较明确，补充条件化操作建议，否则说明观察条件。",
    "comparison": "用户希望对比多个标的，请给出差异、优劣和风险点；用 analyze_multi 并为各标的设合适周期（crypto 4h、黄金等 1d）；对比后若相对强弱或某一标的的单边机会较明确，给出优先标的及条件化操作建议，否则说明双方均宜观望及各自触发条件。",
