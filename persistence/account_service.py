...
def _validate_position_params(symbol: str, entry_price: float, idea_id: str) -> None:
    """校验开仓核心参数，不合法则 raise ValueError 阻止脏数据入库。"""
    if not symbol or not symbol.strip():
        raise ValueError(f"symbol 不能为空")
    if " " in symbol:
        raise ValueError(f"symbol 含空格: {symbol!r}")
    if not idea_id or not idea_id.strip():
        raise ValueError("idea_id 不能为空")
    if idea_id.startswith("idea-pend"):
        raise ValueError(f"idea_id 为占位符: {idea_id!r}")
    if entry_price <= 0:
        raise ValueError(f"entry_price 须为正数, got {entry_price}")


def has_open_position_for_symbol_direction(symbol: str, direction: str) -> bool:
    """检查同一标的+方向是否已有 open 仓位。"""
    eng = _engine()
    if eng is None:
        return False
    with eng.connect() as conn:
        r = conn.execute(
            text("SELECT 1 FROM account_positions WHERE symbol = :sym AND direction = :dir AND status = 'open' LIMIT 1"),
...
def open_position(currency: str, idea: dict[str, Any], fill_qty: float, fill_price: float, order_id: str, now_utc: datetime | None = None) -> None:
    ...
    _validate_position_params(symbol, float(fill_price), idea_id)

    try:
        with eng.begin() as conn:
            # 同标的同方向已有 open 仓位时拒绝开仓，避免叠仓
            existing = conn.execute(
                text("SELECT id, linked_idea_id FROM account_positions WHERE symbol = :sym AND direction = :dir AND status = 'open' LIMIT 1"),
                {"sym": symbol, "dir": direction},
            ).first()
            if existing:
                logger.warning(
                    "[AccountService] open_position 拒绝：标的 {} 方向 {} 已有 open 仓位 id={} idea_id={}",
                    symbol, direction, existing[0], existing[1],
                )
                return

            # 同标的反方向已有 open 仓位时告警但仍允许（ hedge 场景）
            opposite = "short" if direction == "long" else "long"
            opp_existing = conn.execute(
                text("SELECT id, linked_idea_id FROM account_positions WHERE symbol = :sym AND direction = :dir AND status = 'open' LIMIT 1"),
                {"sym": symbol, "dir": opposite},
            ).first()
            if opp_existing:
                logger.warning(
                    "[AccountService] open_position 注意：标的 {} 已有反向仓位 id={} idea_id={}，允许对冲开仓",
                    symbol, opp_existing[0], opp_existing[1],
                )
