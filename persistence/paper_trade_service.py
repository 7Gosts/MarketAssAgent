...
def create_entry_order_and_fill(idea: dict[str, Any], *, now_utc: datetime | None = None) -> None:
    ...
    if not symbol:
        logger.warning("[PaperTrade] skip entry fill: no symbol idea_id={}", idea_id)
        return
    ...
    if fill_px is None:
        ...
        return
    if float(fill_px) <= 0:
        logger.warning("[PaperTrade] skip entry fill: fill_price must be positive, got {} idea_id={}", fill_px, idea_id)
        return
    ...
    if qty_raw_val is None:
        logger.warning("[PaperTrade] skip entry fill: calculated_qty not set idea_id={}", idea_id)
        return
    else:
        try:
            qty = float(qty_raw_val)
        except (TypeError, ValueError):
            logger.warning("[PaperTrade] skip entry fill: calculated_qty invalid idea_id={} val={}", idea_id, qty_raw_val)
            return
    if qty <= 0:
        logger.warning(
