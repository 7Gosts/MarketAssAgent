...
> **历史说明（2026-09-10）**：本文主要记录 2026-06 的改造过程与阶段性描述，包含当时的 LangGraph 表述。当前实际主链路请以 [`00_项目架构.md`](00_项目架构.md) 和 `src/core/agent_loop.py` 为准。

**日期**: 2026-06-27  
**目标读者**: 工程维护者 / 本地执行型 Agent  
**状态**: 已完成并归档：light-only 主链路继续有效，文中的 LangGraph/`core/graph.py` 实施记录已由原生 Agent Loop 替代
**目标**: 把当前“每轮预注入较重 Direct Context”的链路，演进为“轻入口摘要 + LLM 自主按需取证”的循环式主链路，减少短问句被过度上下文化、输出变长和追问判断不稳的问题。

> 当前实现请以 [`00_项目架构.md`](00_项目架构.md) 为准；本文保留迁移过程，不再作为文件路径和框架依赖的现状说明。
...
当时的目标架构保留 LangGraph ReAct loop，不新增独立 Planner 主链路；当前已由 `NativeAgentLoop` 替代：
