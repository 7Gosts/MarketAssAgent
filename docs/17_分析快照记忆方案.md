...
**日期**: 2026-07-10  
**状态**: 已实施；当前使用 PostgreSQL `analysis_snapshots`，由 `analyze_market` 成功后写入
**目标**: 为每次行情分析保存一条轻量结构化快照，让 LLM 在需要“相比上次同标的同周期分析变化”时自主读取，而不是把历史快照每轮硬塞进 prompt。

> **补充更新（2026-09-20）**：`analysis_snapshots` 已统一使用客观事实字段 `ma_regime`，物理列 `trend` 与 `stance` 已删除。`get_previous_analysis_snapshot` 只认数据库，不回退 MemoryAPI。

> **当前补充（2026-09-13）**：调用 `analyze_market` 前会先按会话批量同步全部活跃模拟订单；该前置写操作与分析快照写入均由工具注册层统一编排。
...
- 正式列口径为：`snapshot_id / session_id / source_request_id / symbol / symbol_key / market / provider / interval / snapshot_time / current_price / ma_regime / support_json / resistance_json / payload_json / created_at`。
...
- `symbol`、`interval`、`current_price`、`ma_regime` 至少应存在。
