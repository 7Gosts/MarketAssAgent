...
1. 找出当前会话下全部仍活跃的交易对象，不按本次查询的 `symbol + interval` 缩小范围
2. 读取最新 K 线
3. 用固定规则判断：
   - 是否触发开仓
   - 是否已触发止损/止盈
...
4. 更新数据库当前状态
5. 追加事件流
6. 同批 K 线内继续推进已成交订单，直到没有新变化或进入终态
7. 再让 LLM 回答
...
进入 `watch` 后，是否开仓由代码按普通限价单规则判断：

- 做多挂单：K 线最低价触及或跌破限价
- 做空挂单：K 线最高价触及或突破限价
- 已成交仓位：创建时直接进入 `open / filled`，不经过挂单触发
...
对于 `paper_orders.status='pending_trigger'` 的委托，代码统一按普通限价单判断：

- `side='long'`：`bar.low <= limit_price`
- `side='short'`：`bar.high >= limit_price`
- 历史 `breakout_stop` 标记按普通限价单兼容处理，不再保留突破挂单语义
...
| `order_type` | `varchar(32)` | 是 | 新建普通挂单固定为 `pullback_limit`；历史值仅作存储兼容 |
| `status` | `varchar(24)` | 是 | `pending_trigger / filled / closed / cancelled / expired` |
| `status_reason` | `varchar(64)` | 否 | 当前状态的补充说明 |
| `entry_zone_low` | `numeric(20,8)` | 否 | 入场区下沿 |
| `entry_zone_high` | `numeric(20,8)` | 否 | 入场区上沿 |
| `trigger_price` | `numeric(20,8)` | 否 | 历史兼容字段，新建普通挂单不再写入 |
...
| `ma_regime` | `varchar(24)` | 是 | 机械均线状态：`bullish / bearish / mixed` |
