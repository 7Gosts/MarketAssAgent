...
class AgentRunRequest(BaseModel):
    ...
    default_symbol / default_interval 由 RouteContext 从 session_state + market_config 在运行时推导，
    不再暴露为 API 参数。
    """
...
def run_agent(req: AgentRunRequest) -> AgentRunResponse:
    ...
    agent_request = AgentRequest(
        ...
        text=req.text,
        user_id=req.user_id,
        default_symbol=ROUTER_FALLBACK_SYMBOL,
        default_interval=ROUTER_FALLBACK_INTERVAL,
        context=context,
        options=options,
    )
