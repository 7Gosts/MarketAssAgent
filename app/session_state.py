...
class SessionState:
    ...
    # 待确认意图（多轮交互机制）
    # 当 LLM 返回 clarify 动作时，将原请求结构暂存此处，等待用户补全字段。
    pending_intent: dict[str, Any] | None = None  # 存储诸如 {"action": "analyze", "symbol": "BTC"} 但缺 interval 的意图

    # 最近分析历史（供追问回溯与 clarify 补齐使用，最多保留 5 条）
    ...
    def from_dict(cls, d: dict[str, Any]) -> "SessionState":
        return cls(
            ...
            recent_analyses=list(d.get("recent_analyses") or []),
            # 新增字段
...
def _match_analysis_entry_by_text(text: str, recent_analyses: list[dict[str, Any]]) -> dict[str, Any] | None:
    """从 recent_analyses 中按用户文本匹配分析条目。

    匹配逻辑：
    1. 尝试用 FeishuAssetCatalog.resolve_symbols_from_text() 从文本中提取标的
    2. 命中则匹配包含该标的的最近条目
...
class SessionStateStore:
    ...
    def hydrate(self, session_id: str, state: SessionState) -> None:
        """从持久化层注入会话状态（SessionManager 专用）。"""
        key = str(session_id or state.open_id or "").strip()
        if not key:
            return
        state.open_id = key
        state.updated_ts = float(state.updated_ts or time.time())
        with self._lock:
    ...
    def update_from_route(
        ...
        *,
        ...
        # 追加 recent_analyses 历史
        analysis_actions = {"analyze", "analysis", "quote", "compare", "followup", "analyze_multi", "research"}
        if action in analysis_actions and (symbol or symbols):
            entry = {
                "action": str(action or ""),
                "task_type": str(task_type or ""),
                "symbol": str(symbol or "").strip().upper() if symbol else None,
                "symbols": [str(s).strip().upper() for s in (symbols or [])] if symbols else [],
                "interval": str(interval or "").strip().lower() if interval else None,
                "provider": str(provider or "").strip().lower() if provider else None,
                "output_refs": dict(output_refs or {}),
                "question": str(question or "").strip() if question else None,
            }
            st.recent_analyses.insert(0, entry)
            if len(st.recent_analyses) > 5:
                st.recent_analyses = st.recent_analyses[:5]
    ...
    def resolve_followup_target(self, open_id: str, text: str) -> dict[str, Any]:
        """追问目标解析：优先从 recent_analyses 匹配，再 fallback 到 last_*。

        注意：
        - 只检查有效的分析类 action（analyze / research / quote / compare / followup）
        - 若用户文本中提到了具体标的，优先匹配包含该标的的最近分析条目
        """
        st = self.get(open_id)
        valid_actions = {"analyze", "analysis", "research", "quote", "compare", "followup", "analyze_multi", "sim_account"}

        # 优先从 recent_analyses 匹配
        if st.recent_analyses and text:
            matched = _match_analysis_entry_by_text(text, st.recent_analyses)
            if matched:
                return {
                    "resolved": True,
                    "symbol": matched.get("symbol"),
                    "symbols": matched.get("symbols", []),
                    "interval": matched.get("interval"),
                    "provider": matched.get("provider"),
                    "output_refs": matched.get("output_refs", {}),
                    "last_action": matched.get("action"),
                    "last_task_type": matched.get("task_type"),
                    "last_question": matched.get("question"),
...
def get_global_session_store() -> "SessionManager":
    """全局 SessionManager 单例（组合 SessionStateStore + 文件持久化）。"""
    from app.session_manager import get_global_session_manager

    return get_global_session_manager()


def reset_global_session_store_for_tests() -> None:
    from app.session_manager import reset_global_session_manager_for_tests

    reset_global_session_manager_for_tests()