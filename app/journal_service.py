...
def _handle_filled_event(idea: dict[str, Any], now_utc: datetime) -> None:
    """对 status='filled' 的 idea 执行纸交易入场链（order + fill + position）。

    无论 idea 是「从 watch/pending transition 到 filled」还是「直接创建为 filled」，
    均应调用此函数以确保 paper_orders / paper_fills / account_positions 完整。
    """
    ...
    logger.info(
        "[PositionSizing] symbol={} market={} currency={} entry_ref={} stop={} stop_source={} "
    ...
    if qty <= 0:
        ...
        return
    # 先检查是否已有 entry fill（幂等：重复调用不会重复创建）
    if paper_trade_service.has_entry_fill_for_idea(str(idea.get("idea_id") or "")):
        logger.info("[JournalService] skip _handle_filled_event: entry fill already exists idea_id={}", idea.get("idea_id"))
        return
    # 先执行 position check，提前拦截同方向叠仓，避免 paper fill commit 后无法回滚
    idea_id = str(idea.get("idea_id") or "")
    symbol = str(idea.get("symbol") or "")
    direction = str(idea.get("direction") or "long")
    if account_service.has_open_position_for_symbol_direction(symbol, direction):
        logger.warning("[JournalService] skip _handle_filled_event: 同标的 {} 同方向 {} 已有 open 仓位 idea_id={}", symbol, direction, idea_id)
        return

    paper_trade_service.create_entry_order_and_fill(idea, now_utc=now_utc)
    try:
        order_id = paper_trade_service.stable_order_id(idea_id)
        ...
        logger.exception(
            ...
            "paper fill 已 commit，position 未写入，下次 cycle 不会重试（has_entry_fill=true）",
            idea_id,
        )
    ...
    if journal_created or journal_updated:
        ...
        for idea in journal_new_entries:
            ...
            if str(idea.get("status") or "") == "filled":
                _handle_filled_event(idea, now_utc)
        ...
        for idea_id, evt, _payload in pending_events:
            ...
            if evt == "filled":
                ...
                try:
                    ...
                    logger.exception(
                        "[JournalService] close_position/exit_fill failed for idea_id={}",
                        idea.get("idea_id"),
                    )
            elif evt == "closed_sl":
                try:
