...
from app.formatters.feishu import (
    FEISHU_CARD_SEND_FAILED_MSG,
    build_feishu_delivery,
)
from app.memory_store import JsonlMemoryStore, MemoryEvent
from config.runtime_config import get_analysis_config
from tools.feishu.client import FeishuError, get_tenant_access_token, send_card_message, send_text_message
...
def _feishu_pipeline_log(detail: str) -> None:
    ...
    if v not in {"1", "true", "yes", "on"}:
        return
    logger.info("[FeishuAdapter] pipeline {}", detail)


def load_feishu_settings() -> dict[str, str]:
    """加载飞书配置。

    注：业务默认标的 / 周期已迁移到 runtime route context，
    由 agent_core 从 session_state + market_config + 入口参数推导。
    """
...
def build_event_handler(
    *,
    ...
    def _process_message(*, sender_open_id: str, text: str) -> None:
        ...
        t0 = time.time()
        _feishu_pipeline_log(
            f"core_enter open_id={sender_open_id!s} recent_rounds={llm_memory_rounds}"
        )
        response = handle_request(request)
        elapsed_ms = (time.time() - t0) * 1000.0
        _feishu_pipeline_log(
            f"core_done elapsed_ms={elapsed_ms:.0f} task_type={response.task_type!s} "
            f"reply_chars={len(response.reply_text or '')}"
        )
...
def send_feishu_reply(
    *,
    ...
    delivery = build_feishu_delivery(
    ...
    try:
        ...
        if delivery.kind == "card" and delivery.card:
            try:
                ...
                _feishu_pipeline_log(f"card_sent task_type={response.task_type!s}")
                return
            except FeishuError as exc:
                logger.warning("[FeishuAdapter] card_send_failed err={}", exc)
                send_text_message(
                    tenant_access_token=token,
                    receive_id=sender_open_id,
                    text=FEISHU_CARD_SEND_FAILED_MSG,
                    receive_id_type="open_id",
                )
                return

        text = str(delivery.text or reply_text).strip() or DEFAULT_CHAT_FALLBACK_MESSAGE
        send_text_message(
            tenant_access_token=token,
            receive_id=sender_open_id,
            text=text,
