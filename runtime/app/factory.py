...
from utils.runtime_paths import get_context_dir
...
def create_runtime_services() -> RuntimeServices:
    ...
    agent = MarketReActAgent()
    # User profiles remain local; conversation memory is exclusively the event store.
    memory_api = create_default_memory_api(repo_root=repo_root)
    event_store = LocalSessionEventStore(get_context_dir(repo_root=repo_root))
